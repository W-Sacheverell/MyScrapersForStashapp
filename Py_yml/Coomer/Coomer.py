...
headers = {
    ...
    'Accept': 'text/css'  # ERRO[2026-01-11 20:18:45] [Scrape / Coomer]  Text: If you want to scrape, use "Accept: text/css" header in your requests for now. For whatever reason DDG does not like SPA and JSON, so we have to be funny. And you are no exception to caching.
}
...
def user_query (service, user):
    if re.match('[0-9]*', user): 
        coomer_getuser_url = f"https://coomer.st/api/v1/{service}/user/{user}/profile"
...
def post_query(service, user_id, id):
    coomer_getpost_url = f"https://coomer.st/api/v1/{service}/user/{user_id}/post/{id}"
    ...
    if post_lookup_response.status_code == 200:
        ...
        out = {"Title": post['title'],
               ...
               "URL": f"https://coomer.st/{post['service']}/user/{post['user']}/post/{post['id']}",
...
def sceneByFragment(fragment):
    ...
    coomer_searchhash_url = "https://coomer.st/api/v1/search_hash/"
